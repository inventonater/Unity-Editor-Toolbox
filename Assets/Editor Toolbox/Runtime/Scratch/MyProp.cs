...
{
    [Serializable]
    public class MyProp<T> : ReactiveProperty<T>, ISerializationCallbackReceiver
    {
        [SerializeField]
        T value;
        ...
        void ISerializationCallbackReceiver.OnAfterDeserialize()
        {
            ...
            Debug.Log($"OnAfterDeserialize {(!IsDisposed && Value != null ? Value.GetHashCode() : -1)}");
        }
    }
    ...
    internal class MyPropDrawer : PropertyDrawer
    {
        ...
        object GetValueRecursive(object obj, int index, string[] paths)
        {
            ...
            if (fldInfo == null && path == "Array")
            {
                ...
                {
                    Debug.Log("MyPropDrawer Exception, objType:" + obj.GetType().Name + " path:" + string.Join(", ", paths));
                    throw;
                }
            }
